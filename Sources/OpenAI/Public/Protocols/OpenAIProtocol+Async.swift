...
public extension OpenAIProtocol {
    ...
    func completionsStream(query: CompletionsQuery) -> AsyncThrowingStream<CompletionsResult, Error> {
    ...
    func chatsVision(query: ChatVisionQuery) async throws -> ChatResult {
        try await withCheckedThrowingContinuation { continuation in
            chatsVision(query: query) { result in
                switch result {
                case let .success(success):
                    return continuation.resume(returning: success)
                case let .failure(failure):
    ...
    func audioSpeech(query: AudioSpeechQuery) async throws -> Data {
        try await withCheckedThrowingContinuation { continuation in
    ...
    func audioSpeechStream(query: AudioSpeechQuery) -> AsyncThrowingStream<Data, Error> {
        return AsyncThrowingStream { continuation in
            return audioSpeechStream(query: query)  { result in
                continuation.yield(with: result)
            } completion: { error in
                continuation.finish(throwing: error)
            }
